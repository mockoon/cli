...
describe('Dockerize command', () => {
  test
    ...
    .it('should successfully run the command', (context) => {
      ...
      expect(context.stdout).to.contain('docker build -t mockoon-cli-mock1 .');
      expect(context.stdout).to.contain(
        'docker run -d -p 3010:3010 mockoon-cli-mock1'
      );
    });
  ...
  test.it(
    ...
    async () => {
      ...
      expect(dockerfileContent).to.contain('COPY mock1.json ./data');
  ...
  test.it('should generate mock JSON file next to the Dockerfile', async () => {
    ...
    expect(mockFile.name).to.equal('mock1');
